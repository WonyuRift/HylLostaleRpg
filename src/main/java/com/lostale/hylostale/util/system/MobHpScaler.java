...
public class MobHpScaler {

    public static void apply(Store<EntityStore> store, CommandBuffer<EntityStore> cb, Ref<EntityStore> mobRef, int lvl) {

        EntityStatMap map = (EntityStatMap) store.getComponent(mobRef, EntityStatMap.getComponentType());
        if (map == null) return;
        ...
        float newMax = baseMax * (1f + 0.08f * Math.max(0, lvl - 1));
